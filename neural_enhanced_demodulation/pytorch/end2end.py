...
def checkpoint(iteration, mask_CNN, C_XtoY, opts):
    ...
    mask_CNN_path = os.path.join(
        opts.checkpoint_dir, str(iteration) + '_maskCNN.pkl')
    torch.save(mask_CNN.state_dict(), mask_CNN_path)

    C_XtoY_path = os.path.join(
        opts.checkpoint_dir, str(iteration) + '_C_XtoY.pkl')
...
def load_checkpoint(opts):
    ...
    maskCNN_path = os.path.join(
        opts.checkpoint_dir, str(opts.load_iters) + '_maskCNN.pkl')
    ...
    C_XtoY_path = os.path.join(
        opts.checkpoint_dir, str(opts.load_iters) + '_C_XtoY.pkl')
...
def save_samples(iteration, fixed_Y, fixed_X, mask_CNN, opts):
    ...
    merged = merge_images(
        fixed_X, fake_Y, opts.batch_size, opts.y_image_channel)

    path = os.path.join(opts.sample_dir,
                        'sample-{:06d}-Y.png'.format(iteration))
    merged = np.abs(merged[:, :, 0] + 1j * merged[:, :, 1])
    merged = (merged - np.amin(merged)) / \
        (np.amax(merged) - np.amin(merged)) * 255
...
def save_samples_separate(iteration, fixed_Y, fixed_X, mask_CNN, opts,
    ...
    fixed_Y, fake_Y, fixed_X = to_data(
        fixed_Y), to_data(fake_Y), to_data(fixed_X)
    ...
    for batch_index in range(opts.batch_size):
        if batch_index < len(name_X_test):
            ...
            groundtruth_image = np.abs(
                groundtruth_image[:, :, 0] + 1j * groundtruth_image[:, :, 1])
            groundtruth_image = (groundtruth_image - np.amin(groundtruth_image)) / (
                np.amax(groundtruth_image) - np.amin(groundtruth_image)) * 255
            cv2.imwrite(path_src + '_groundtruth_' +
                        str(iteration) + '.png', groundtruth_image)

            fake_image = (
                np.squeeze(fake_Y[batch_index, :, :, :]).transpose(1, 2, 0))
            fake_image = np.abs(fake_image[:, :, 0] + 1j * fake_image[:, :, 1])
            fake_image = (fake_image - np.amin(fake_image)) / \
                (np.amax(fake_image) - np.amin(fake_image)) * 255
            cv2.imwrite(path_src + '_fake_' +
                        str(iteration) + '.png', fake_image)

            raw_image = (
                np.squeeze(fixed_X[batch_index, :, :, :]).transpose(1, 2, 0))
            raw_image = np.abs(raw_image[:, :, 0] + 1j * raw_image[:, :, 1])
            raw_image = (raw_image - np.amin(raw_image)) / \
                (np.amax(raw_image) - np.amin(raw_image)) * 255
            cv2.imwrite(path_src + '_raw_' +
                        str(iteration) + '.png', raw_image)
...
def training_loop(training_dataloader_X, training_dataloader_Y, testing_dataloader_X,
    ...
    for iteration in range(iter_per_epoch_test):
        ...
        images_X_test_spectrum_raw = torch.stft(input=images_X_test, n_fft=opts.stft_nfft,
                                                ...
                                                pad_mode='constant')
        images_X_test_spectrum = spec_to_network_input(
            images_X_test_spectrum_raw, opts)

        images_Y_test_spectrum_raw = torch.stft(input=images_Y_test, n_fft=opts.stft_nfft,
                                                hop_length=opts.stft_overlap, win_length=opts.stft_window,
                                                pad_mode='constant')
        images_Y_test_spectrum = spec_to_network_input(
            images_Y_test_spectrum_raw, opts)
        ...
        for batch_index in range(opts.batch_size):
            ...
            try:
                ...
                error_matrix_info.append([instance_X_test_mapping[batch_index], code_X_test_mapping[batch_index],
                                          snr_X_test_mapping[batch_index],
                                          labels_X_test_estimated[batch_index].cpu(
                ).data.int(),
                    labels_X_test[batch_index].cpu().data.int()])
            except Exception as e:
                print('length of error_matrix:', len(error_matrix), ' index was:', snr_index)
                print(len(snr_X_test_mapping), batch_index)
                print(len(test_right_case), batch_index)
                print(len(instance_X_test_mapping), batch_index)
                print(len(code_X_test_mapping), batch_index)
                print(len(snr_X_test_mapping), batch_index)
                print(len(labels_X_test_estimated), batch_index)
                print(len(labels_X_test), batch_index)
                print("Something else went wrong, exception was:", e)
        if iteration % opts.log_step == 0:
